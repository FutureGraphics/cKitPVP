...
public class Kits implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length == 0) {
			...
			StringBuilder otherkits = new StringBuilder();
			for(String kit : Settings.kits) {
				if(p.isOp() || p.hasPermission("ckitpvp.kit." + kit) || pd.unlockedkits().contains(kit)) {
					...
					otherkits.append(kit);
				}
				
			}
			if(kits.length() != 0) kits = new StringBuilder(kits.substring(0, kits.length() - 2));
			if(otherkits.length() != 0) otherkits = new StringBuilder(otherkits.substring(0, otherkits.length() - 2));
